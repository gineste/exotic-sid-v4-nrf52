 ...
 * Defines
 ************************************************************************/
#define SENSOR_POWERDOWN_TIME (uint32_t) 8000u
...
{   
   ...
   vGPIO_InputCfg(ST25DV_INT, HALGPIO_PIN_NOPULL);
   vGPIO_InputCfg(LSM6_INT1, HALGPIO_PIN_NOPULL);
   vGPIO_InputCfg(LSM6_INT2, HALGPIO_PIN_NOPULL);
   
   vGPIO_OutputCfg(VSEL, HALGPIO_PIN_NOPULL);
   #ifdef DEBUG   /* Issue with JLINK, did not support Vdd boost */
      vHal_GPIO_Clear(VSEL);   
   #else
      vHal_GPIO_Set(VSEL);   
   #endif

   /* UART */
   vGPIO_InputCfg(IRQ_CC, HALGPIO_PIN_PULLDOWN);
   vGPIO_InputCfg(INT_CC, HALGPIO_PIN_PULLDOWN);
